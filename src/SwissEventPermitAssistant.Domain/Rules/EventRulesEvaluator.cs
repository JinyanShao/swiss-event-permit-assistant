...
public sealed class EventRulesEvaluator
{
    ...
    private void AddPoliceLocaleRule(EventProfile profile, List<ActionRequirement> actions, List<ConfirmationItem> confirmations, List<Deadline> deadlines, HashSet<string> sourceIds)
    {
        ...
        actions.AddUnique(new ActionRequirement("ACT-POLICE-LOCALE", "Autorisation Police locale", RequirementStatus.Required, "Ville de Fribourg - Police locale", "La manifestation se trouve dans le périmètre Ville de Fribourg V0.1.", sourceId, deadline));
    }
    ...
    private void AddPatenteKRules(EventProfile profile, List<ActionRequirement> actions, List<DocumentRequirement> documents, List<InformationItem> information, List<ConfirmationItem> confirmations, List<Deadline> deadlines, HashSet<string> sourceIds)
    {
        ...
        if (patenteRequired)
        {
            ...
            {
                documents.AddMerge(new DocumentRequirement("DOC-OWNER-AUTHORIZATION", "Autorisation signée du propriétaire", RequirementStatus.Required, "Nécessaire pour une Patente K sur fonds privé.", ["R-OWNER-001"], "SRC-FR-PATENTE-K"));
            }
        }

        if (profile.BeverageMode is BeverageMode.BeveragesFree or BeverageMode.NotSure)
        {
            AddConfirmation(confirmations, "CONF-FREE-BEVERAGES-PATENTE", "Patente K pour boissons gratuites", "Les sources publiques consultées confirment la vente de boissons, mais pas clairement la distribution gratuite.", "Confirmer avec la Préfecture si des boissons sont offertes.", "Préfecture du district", "SRC-FR-PATENTE-K", sourceIds);
        }

        if (profile.FoodMode is FoodMode.FoodFree or FoodMode.NotSure)
        {
            AddConfirmation(confirmations, "CONF-FREE-FOOD-PATENTE", "Patente K pour nourriture gratuite", "Les sources publiques consultées confirment la vente de mets, mais pas clairement la distribution gratuite.", "Confirmer avec la Préfecture si de la nourriture est offerte.", "Préfecture du district", "SRC-FR-PATENTE-K", sourceIds);
        }

        if (profile.AlcoholMode is AlcoholMode.AlcoholFree or AlcoholMode.NotSure)
        {
            AddConfirmation(confirmations, "CONF-FREE-ALCOHOL-PATENTE", "Patente K pour alcool offert", "Les sources publiques consultées ne permettent pas de confirmer automatiquement si l’alcool offert gratuitement exige une Patente K.", "Confirmer avec la Préfecture avant de déposer le dossier.", "Préfecture du district", "SRC-FR-PATENTE-K", sourceIds);
        }

        if (profile.AlcoholMode is AlcoholMode.AlcoholSold or AlcoholMode.AlcoholFree or AlcoholMode.NotSure)
        {
            information.AddUnique(new InformationItem("INFO-ALCOHOL", "Boissons alcoolisées", "Prévoir les informations et obligations de prévention liées aux boissons alcoolisées.", "SRC-FR-PATENTE-K"));
            Use(sourceIds, "SRC-FR-PATENTE-K");
        }
        ...
        {
            AddConfirmation(confirmations, "CONF-FORM-B", "Formulaire B", "Le Formulaire B concerne les manifestations d’importance, mais aucun seuil objectif public n’a été confirmé pour V0.1.", "Ne pas le considérer automatiquement requis; vérifier si l’autorité le demande.", "Préfecture du district", "SRC-FR-FORM-B", sourceIds);
        }
    }
    ...
    private static void AddSmartAndReuseRules(EventProfile profile, List<ActionRequirement> actions, List<InformationItem> information, List<ConfirmationItem> confirmations, List<Deadline> deadlines, HashSet<string> sourceIds)
    {
        ...
        {
            AddConfirmation(confirmations, "CONF-VENUE", "Lieu à confirmer", "Le type de lieu détermine certaines obligations liées à l’espace public.", "Confirmer si la manifestation utilise l’espace public.", "Ville de Fribourg - Police locale", "SRC-VDF-LT200", sourceIds);
        }

        if (profile.VenueKind == VenueKind.PrivateVenue && profile.IsPublicEvent == YesNoUnknown.Yes)
        {
            AddConfirmation(confirmations, "CONF-PRIVATE-POLICE", "Autorisation Police locale sur terrain privé", "Les sources publiques mentionnent les manifestations publiques sur fonds privé, mais ne permettent pas de confirmer automatiquement le besoin d’autorisation.", "Contacter la Police locale avant de déposer le dossier.", "Ville de Fribourg - Police locale", "SRC-VDF-LT200", sourceIds);
        }
    }
    ...
    private static void AddSetupRules(EventProfile profile, List<DocumentRequirement> documents, List<InformationItem> information, List<ConfirmationItem> confirmations, HashSet<string> sourceIds)
    {
        ...
        {
            documents.AddMerge(new DocumentRequirement("DOC-SITE-PLAN", "Plan du site", RequirementStatus.Required, "Nécessaire pour décrire les installations et les accès.", ["R-INSTALLATION-001"], "SRC-FR-FORM-B"));
            documents.AddMerge(new DocumentRequirement("DOC-INSTALLATION-DESCRIPTION", "Description des installations temporaires", RequirementStatus.Required, "Tente, scène, bar, cuisine, WC ou installations similaires.", ["R-INSTALLATION-001"], "SRC-FR-FORM-B"));
            Use(sourceIds, "SRC-FR-FORM-B");
        }
        else if (profile.HasTemporaryInstallations == YesNoUnknown.Unknown)
        {
            AddConfirmation(confirmations, "CONF-INSTALLATIONS", "Installations temporaires", "Cette réponse peut changer les documents à préparer.", "Confirmer si des installations temporaires sont prévues.", "Ville de Fribourg - Police locale", "SRC-FR-FORM-B", sourceIds);
        }

        if (profile.HasAmplifiedMusicOrSound != YesNoUnknown.No)
        {
            information.AddUnique(new InformationItem("INFO-SOUND", "Musique et sonorisation", "La musique amplifiée ou la sonorisation peut entraîner des conditions liées au bruit ou aux horaires.", SourceForAttendance(profile.ExpectedAttendance)));
            Use(sourceIds, SourceForAttendance(profile.ExpectedAttendance));

            if (profile.HasAmplifiedMusicOrSound == YesNoUnknown.Unknown || profile.EventEndTime is not null)
            {
                AddConfirmation(confirmations, "CONF-SOUND", "Conditions liées au son", "Les conditions exactes dépendent du lieu, de l’horaire et de l’autorité compétente.", "Vérifier les conditions applicables avec la Police locale ou l’autorité compétente.", "Ville de Fribourg - Police locale", SourceForAttendance(profile.ExpectedAttendance), sourceIds);
            }
        }

        if (profile.UsesGasGrillOrHeater != YesNoUnknown.No)
        {
            information.AddUnique(new InformationItem("INFO-GAS", "Gril ou chauffage à gaz", "Prévoir le respect des normes GPL mentionnées par la Ville.", SourceForAttendance(profile.ExpectedAttendance)));
            Use(sourceIds, SourceForAttendance(profile.ExpectedAttendance));
        }
    }
    ...
    private void AddMobilityRules(EventProfile profile, List<ActionRequirement> actions, List<DocumentRequirement> documents, List<ConfirmationItem> confirmations, List<Deadline> deadlines, HashSet<string> sourceIds)
    {
        ...
        {
            documents.AddMerge(new DocumentRequirement("DOC-SITE-PLAN", "Plan du site", RequirementStatus.Required, "Utile pour documenter les accès, la circulation et l’organisation du site.", ["R-TRAFFIC-001"], "SRC-FR-FORM-B"));
            ...
            AddConfirmation(confirmations, "CONF-TRAFFIC", "Mesures de circulation", "Les mesures exactes doivent être validées par l’autorité compétente.", "Confirmer les besoins de signalisation, fermeture ou stationnement.", "Police cantonale / Police locale", "SRC-FR-FORM-B", sourceIds);
        }
        else if (profile.AffectsTrafficOrParking == YesNoUnknown.Unknown)
        {
            AddConfirmation(confirmations, "CONF-TRAFFIC-UNKNOWN", "Impact circulation à confirmer", "Un impact sur la circulation peut changer les documents et autorisations.", "Vérifier si la manifestation influence la circulation ou le stationnement.", "Police locale", "SRC-FR-FORM-B", sourceIds);
        }

        if (profile.HasProcessionOrRoute == YesNoUnknown.Yes)
        {
            documents.AddMerge(new DocumentRequirement("DOC-ROUTE-PLAN", "Plan du parcours", RequirementStatus.Required, "Nécessaire pour un cortège, parcours ou occupation de route.", ["R-ROUTE-001"], "SRC-FR-FORM-B"));
            AddConfirmation(confirmations, "CONF-ROAD-CLOSURE", "Fermeture ou restriction de route", "Une fermeture ou restriction peut nécessiter une validation spécifique.", "Confirmer avec l’autorité compétente.", "Police locale / Police cantonale", "SRC-FR-FORM-B", sourceIds);
        }
        else if (profile.HasProcessionOrRoute == YesNoUnknown.Unknown)
        {
            AddConfirmation(confirmations, "CONF-ROUTE-UNKNOWN", "Parcours à confirmer", "Un parcours ou cortège peut changer les documents à préparer.", "Confirmer si un parcours est prévu.", "Police locale", "SRC-FR-FORM-B", sourceIds);
        }
        ...
        if (profile.IsSportCompetitionOnPublicRoad == YesNoUnknown.Yes)
        {
            ...
            deadlines.AddUnique(deadline);
            actions.AddUnique(new ActionRequirement("ACT-OCN-SPORT", "Autorisation OCN", RequirementStatus.Required, "Office de la circulation et de la navigation", "Vous avez indiqué une compétition sportive sur la voie publique.", "SRC-OCN-SPORT", deadline));
    ...
    {
        documents.AddMerge(new DocumentRequirement("DOC-LIABILITY-INSURANCE", "Assurance responsabilité civile valable", RequirementStatus.Required, "La Ville indique que l’organisateur doit pouvoir présenter une RC valable.", ["R-RC-001"], SourceForAttendance(profile.ExpectedAttendance)));
        Use(sourceIds, SourceForAttendance(profile.ExpectedAttendance));

        if (profile.HasLiabilityInsurance != YesNoUnknown.Yes)
        {
            AddConfirmation(confirmations, "CONF-RC", "Assurance responsabilité civile", "L’assurance RC n’est pas confirmée dans vos réponses.", "Prévoir une RC valable ou vérifier ce point avant de déposer la demande.", "Ville de Fribourg", SourceForAttendance(profile.ExpectedAttendance), sourceIds);
        }
    }
    ...
    private void AddOptionalVilleRules(EventProfile profile, List<ActionRequirement> actions, List<Deadline> deadlines, HashSet<string> sourceIds)
    {
        ...
        {
            var deadline = CreateDaysBefore("DL-MATERIAL-30", "Matériel ou décorations communales", profile.EventDate, 30, SourceForAttendance(profile.ExpectedAttendance));
            deadlines.AddUnique(deadline);
            actions.AddUnique(new ActionRequirement("ACT-MUNICIPAL-MATERIAL", "Matériel ou décorations communales", RequirementStatus.Required, "Ville de Fribourg", "Vous avez indiqué un besoin de matériel ou de décorations communales.", SourceForAttendance(profile.ExpectedAttendance), deadline));
            Use(sourceIds, SourceForAttendance(profile.ExpectedAttendance));
        }
        ...
        if (profile.NeedsAdvertisingBannerOrPublicPosting)
        {
            ...
            deadlines.AddUnique(deadline);
            actions.AddUnique(new ActionRequirement("ACT-ADVERTISING", "Affichage public ou banderole", RequirementStatus.Required, "Ville de Fribourg - Police locale", "Vous avez indiqué un besoin d’affichage public ou de banderole.", SourceForAttendance(profile.ExpectedAttendance), deadline));
            Use(sourceIds, SourceForAttendance(profile.ExpectedAttendance));
        }
    }
